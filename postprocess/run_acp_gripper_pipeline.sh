...
# push_0728 -> ACP raw -> zarr -> virtual-target labels -> optional 20D training.
# This script uses adaptive_compliance_policy_extend and does not alter the old dataset.
...
DATASET_NAME="push_0728_gripper_acp_v1"
...
if [[ "$RUN_RAW_CONVERT" == "true" ]]; then
  command=(
    "$PYTHON_BIN" "$SCRIPT_DIR/convert_acp_gripper_raw.py"
  ...
  echo "Step 1/4: Data_Collect -> ACP raw with gripper/tactile"
  "${command[@]}"
fi
...
if [[ "$RUN_TRAIN" == "true" ]]; then
  echo "Step 4/4: train 20D ACP (last dimension is gripper open/close)"
  (
