...
# Recommended push_0728 v2 pipeline. It preserves v1 and skips multi-cycle retakes.
...
if [[ "$RUN_RAW_CONVERT" == "true" ]]; then
  command=(
    "$PYTHON_BIN" "$SCRIPT_DIR/convert_acp_gripper_command_single_cycle_raw.py"
  ...
  echo "Step 1/4: infer causal gripper commands from single-cycle episodes"
  "${command[@]}"
fi
